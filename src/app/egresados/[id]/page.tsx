...
import { ArrowLeft, Pencil, Phone, MapPin, Calendar, Briefcase, Building2 } from "lucide-react";
...
export default async function EgresadoDetallePage({ params }: { params: { id: string } }) {
  ...
  .from(egresado).leftJoin(planEstudios, eq(egresado.idPlan, planEstudios.id))
    ...
    <AdminLayout correo={session.correo}>
      <div className="space-y-6 animate-fade-up">
        <div className="flex items-center gap-3">
          ...
          {/* Botón editar apuntando a la ruta correcta */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-4">
            <div className="card text-center">
              <div className="w-20 h-20 rounded-2xl bg-primary-600/20 border-2 border-primary-500/30
                ...
                <span className="text-primary-300 text-2xl font-bold">
                  {eg.apellidos[0]}{eg.nombres[0]}
                </span>
              </div>
              <h2 className="text-white font-bold text-lg">{eg.apellidos}, {eg.nombres}</h2>
            ...
            <div className="card space-y-3">
              ...
              {eg.telefono && (
                <div className="flex gap-3 text-sm">
                  ...
                  <span className="text-slate-300">{eg.telefono}</span>
                </div>
              )}
              {eg.direccion && (
                <div className="flex gap-3 text-sm">
            ...
            <div className="card space-y-2">
              ...
              <p className="text-slate-300 text-sm"><span className="text-slate-500">Plan:</span> {eg.nombrePlan}</p>
              <p className="text-slate-300 text-sm"><span className="text-slate-500">Graduación:</span> {fmtDate(eg.fechaGraduacion)}</p>
              <p className="text-slate-600 text-xs mt-2">Registrado: {fmtDate(eg.fechaRegistro?.toISOString())}</p>
            </div>
          </div>
